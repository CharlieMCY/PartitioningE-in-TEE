...
gt_o = [116, 117, 118, 119, 121, 122, 126, 136, 137, 138, 139, 143, 144, 148, 154, 155, 175, 176, 177, 178, 184, 185, 186, 190, 191]
gt_i = [201, 202, 203, 205, 208, 210, 212, 220, 221, 222, 224, 227, 231, 238, 254, 256, 258, 283, 284, 285, 287, 290, 291, 295, 296, 304, 306, 307, 309]
gt_s = [319, 324, 329, 334, 338, 340, 357, 359, 366, 372, 378, 384, 389, 393, 422, 425, 432, 437, 443, 445, 451, 457, 463, 468, 472]
...
# def imem_query(funcs, iflist, l_sec, c_i, l_i):
#     params_i = {}

#     codeql_run = "codeql query run ..//..//query//memflow.ql " + \
#             "--database=tee_example > " + \
#             "out//memflow.out"
#     os.system(codeql_run)

#     if os.path.exists("out//memflow.out"):
#         f = open("out//memflow.out", "r")
#         all_lines = f.readlines()
#         for fname in all_lines[2:]:
#             tmp = fname.replace(" ", "").split('|')
#             if funcs.has_key('callto' + tmp[4]):
#                 current_func = funcs['callto' + tmp[4]]
#             else:
#                  current_func = 'callto' + tmp[4]
#             key = tmp[2] + '_' + tmp[1] + '_' + current_func
            
#             if tmp[5] in iflist:
#                 tmp[3] = tmp[3] + 'if'

#             if params_i.has_key(key):
#                 params_i[key].append(tmp[3] + tmp[5])
#             else:
#                 params_i[key] = []
#                 params_i[key].append(tmp[3] + tmp[5])
...
#         for key in params_i:
#             check = ''
#             for item in params_i[key]:
#                 check = check + item
#                 if 'MemMove' in item or 'snprintf' in item:
#                     if not (('if' in check) or (item in l_sec) or (item in l_i)):
#                         c_i = c_i + 1
#                         l_i.append(item)
...
# c_i, l_i = imem_query(funcs, iflist, l_sec, c_i, l_i)
